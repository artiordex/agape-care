...
export default function StaffManagement() {
  ...
  return (
    <div className="p-6">
      <div className="mb-6">
        <h1 className="text-2xl font-bold text-gray-800 mb-2">직원 관리</h1>
        <p className="text-gray-600 text-sm">직원 정보, 자격증, 근무표, 근태를 관리합니다</p>
      </div>
      ...
      <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6 mb-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
          <div className="p-4 bg-gradient-to-br from-blue-50 to-blue-100 rounded-lg">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-2xl font-bold text-blue-700">{staffList.filter(s => s.status === '재직').length}명</p>
              </div>
          ...
          <div className="p-4 bg-gradient-to-br from-orange-50 to-orange-100 rounded-lg">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-sm text-orange-600 font-medium">자격증 만료 임박</p>
                <p className="text-2xl font-bold text-orange-700">
                  {staffList.filter(s => {
                    const diffDays = Math.ceil((new Date(s.certExpiry).getTime() - new Date().getTime()) / (1000 * 60 * 60 * 24));
          ...
          <div className="p-4 bg-gradient-to-br from-green-50 to-green-100 rounded-lg">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-sm text-green-600 font-medium">오늘 근무자</p>
              ...
              {filteredStaff.map((staff, index) => {
                ...
                return (
                  <tr key={staff.id} className={index % 2 === 0 ? 'bg-white' : 'bg-gray-50'}>
                    <td className="px-4 py-3 text-sm text-gray-800 font-medium">{staff.name}</td>
                    ...
                    <td className="px-4 py-3">
                      ...
                      <span className={`inline-block px-2 py-0.5 text-xs font-medium rounded-full ${certStatus.color} mt-1`}>
                        {certStatus.text}
                      </span>
                    </td>
                    <td className="px-4 py-3">
...
function StaffModal({ staff, onClose, onSave }: { staff: Staff | null; onClose: () => void; onSave: (data: Partial<Staff>) => void }) {
      ...
      <div className="bg-white rounded-lg shadow-xl max-w-2xl w-full max-h-[90vh] overflow-y-auto">
        ...
        <form onSubmit={handleSubmit} className="p-6">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
            ...
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-2">자격증 번호</label>
              <input
                type="text"
            ...
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-2">자격증 만료일</label>
              <input
                type="date"
            ...
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-2">연차 일수</label>
              <input
                type="number"
...
function WorkScheduleModal({ staffList, currentWeek, onClose, onSave }: {
  staffList: Staff[];
  currentWeek: Date;
  onClose: () => void;
            ...
            <table className="w-full border-collapse">
              <thead>
                <tr className="bg-gray-50">
                  <th className="px-4 py-3 text-left text-sm font-semibold text-gray-700 border">직원명</th>
                  {weekDates.map((date, index) => (
                    <th key={index} className="px-4 py-3 text-center text-sm font-semibold text-gray-700 border">
              ...
              <tbody>
                {staffList.filter(s => s.status === '재직').map((staff) => (
                  <tr key={staff.id} className="hover:bg-gray-50">
                    <td className="px-4 py-3 text-sm font-medium text-gray-800 border">
                      {staff.name}
                      ...
                      return (
                        <td key={index} className="px-2 py-2 border">
                          <select
                            ...
                            className="w-full px-2 py-1 text-sm border border-gray-300 rounded cursor-pointer focus:ring-2 focus:ring-teal-500 focus:border-transparent"
                          >
