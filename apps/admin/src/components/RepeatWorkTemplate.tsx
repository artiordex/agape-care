...
export default function RepeatWorkTemplate() {
  ...
  const handleDelete = (id: string) => {
    if (confirm('이 반복근무 템플릿을 삭제하시겠습니까?')) {
      const updated = templates.filter(t => t.id !== id);
      saveTemplates(updated);
    }
  };

  const handleApplyTemplate = (template: RepeatTemplate) => {
    // Fixed syntax error by using string concatenation instead of a template literal
  ...
  return (
    <div className="p-6">
      {/* 헤더 */}
      <div className="mb-6">
        <h1 className="text-2xl font-bold text-gray-800 mb-2">반복근무 등록</h1>
        <p className="text-gray-600 text-sm">일정한 패턴으로 반복되는 근무 템플릿을 관리합니다</p>
      </div>
        ...
        {templates.map(template => (
          <div key={template.id} className="bg-white rounded-lg shadow-sm border border-gray-200 overflow-hidden">
            ...
            <div className="p-4">
              ...
              {/* 패턴 */}
              <div className="mb-4">
                <div className="text-sm font-medium text-gray-700 mb-3">근무 패턴</div>
                    ...
                    return (
                      <div key={index} className="flex items-center gap-3 p-2 bg-gray-50 rounded-lg">
                        <div className="w-16 text-center">
                          <span className="text-sm font-bold text-gray-700">{p.day}일차</span>
                        </div>
                        {workCode && (
                          <>
              ...
              <button
                ...
                className="w-full px-4 py-2 bg-gradient-to-r from-teal-500 to-cyan-500 text-white rounded-lg hover:shadow-lg transition-all duration-300 whitespace-nowrap cursor-pointer"
              >
      ...
      {templates.length === 0 && (
        <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-12 text-center">
          ...
          <p className="text-gray-500 mb-4">등록된 반복근무 템플릿이 없습니다</p>
          <button
            onClick={() => {
              setEditingTemplate(null);
              setShowModal(true);
...
function RepeatTemplateModal({
  ...
  onClose: () => void;
  onSave: (template: RepeatTemplate) => void;
}) {
  ...
  const [pattern, setPattern] = useState(
    template?.pattern || [
      { day: 1, workType: 'S', startTime: '09:00', endTime: '18:00', breakTime: 60 },
      { day: 2, workType: 'S', startTime: '09:00', endTime: '18:00', breakTime: 60 },
      { day: 3, workType: '휴', startTime: '', endTime: '', breakTime: 0 },
      { day: 4, workType: 'N', startTime: '20:00', endTime: '08:00', breakTime: 60 },
      { day: 5, workType: '휴', startTime: '', endTime: '', breakTime: 0 },
      { day: 6, workType: '휴', startTime: '', endTime: '', breakTime: 0 },
    ]
  );

  const updatePatternDays = (days: number) => {
    const newPattern = [];
    for (let i = 1; i <= days; i++) {
      const existing = pattern.find(p => p.day === i);
      newPattern.push(
        existing || { day: i, workType: 'A', startTime: '09:00', endTime: '18:00', breakTime: 60 }
      );
    }
  ...
  const handleSubmit = () => {
    ...
    const newTemplate: RepeatTemplate = {
      ...
      <div className="bg-white rounded-lg shadow-xl max-w-4xl w-full max-h-[90vh] overflow-y-auto">
        <div className="sticky top-0 bg-white border-b border-gray-200 px-6 py-4 flex items-center justify-between">
          <h3 className="text-xl font-bold text-gray-800">
            {template ? '반복근무 템플릿 수정' : '새 반복근무 템플릿'}
          </h3>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
            <div className="md:col-span-2">
              ...
              <input
                ...
                placeholder="예: 주야비비비 패턴"
                className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-purple-500 focus:border-transparent"
              />
            </div>

            <div>
              <label className="block text-sm font-medium text-gray-700 mb-2">반복 일수 *</label>
              <input
                type="number"
                min="1"
                max="31"
            ...
            <div>
              ...
              <input
                ...
                className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-purple-5
...
0 focus:border-transparent"
          ...
          {/* 패턴 설정 */}
          <div className="mb-6">
            <label className="block text-sm font-medium text-gray-700 mb-3">근무 패턴 설정</label>
                ...
                <div key={index} className="flex items-center gap-3 bg-white p-3 rounded-lg border border-gray-200">
                  ...
                  <select
                    ...
                    onChange={(e) => {
                      ...
                      newPattern[index].workType = e.target.value;
                      setPattern(newPattern);
                    }}
                    className="px-3 py-2 border border-gray-300 rounded-lg text-sm focus:ring-2 focus:ring-purple-500 cursor-pointer"
                  >
                    ...
                    <>
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        <input
                          ...
                          onChange={(e) => {
                            const newPattern = [...pattern];
                            newPattern[index].breakTime = parseInt(e.target.value) || 0;
                            setPattern(newPattern);
                          }}
                          className="w-20 px-2 py-2 border border-gray-300 rounded-lg text-sm focus:ring-2 focus:ring-purple-500"
                        />
