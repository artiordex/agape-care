// app/settings/RBACManagement.tsx
'use client';
    ...
    menus: [
      {
        menuId: 'notice',
        menuName: '공지사항',
        screens: [{ screenId: 'notice_list', screenName: '공지사항 관리', actions: allActions }],
      },
      {
        menuId: 'freeboard',
        menuName: '자유게시판',
        screens: [{ screenId: 'freeboard_list', screenName: '자유게시판 관리', actions: allActions }],
      },
      {
        menuId: 'gallery',
        menuName: '갤러리',
        screens: [{ screenId: 'gallery_list', screenName: '갤러리 관리', actions: allActions }],
      },
    ],
  },
  {
...
export default function RBACManagement() {
  ...
  const filteredEmployees = employees.filter(
    emp => emp.name.includes(searchQuery) || emp.position.includes(searchQuery),
  );
  ...
  const totalScreens = allPermissions.reduce(
    (sum, cat) => sum + cat.menus.reduce((mSum, menu) => mSum + menu.screens.length, 0),
    0,
  );
    ...
    return (
      <div className="flex h-full items-center justify-center">
        <div className="text-center">
          ...
          <p className="mt-4 text-gray-600">권한 관리 로딩 중...</p>
        </div>
      </div>
    );
  }
      ...
      <div className="border-b border-gray-200 bg-white px-6 py-4">
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="mt-1 text-sm text-gray-500">직원별 세부 권한을 설정하고 관리합니다</p>
          </div>
        </div>
      </div>
            ...
            <>
              ...
              <div className="flex-1 overflow-y-auto">
                {filteredEmployees.map(emp => (
                  <div
                    ...
                    className={`cursor-pointer border-b border-gray-100 p-4 transition-colors ${
                      selectedEmployee === emp.id ? 'border-l-4 border-l-teal-500 bg-teal-50' : 'hover:bg-gray-50'
                    }`}
                  >
                    <div className="flex items-center justify-between">
                      <div className="flex-1">
                        <div className="flex items-center gap-2">
                          ...
                          <span className="rounded bg-blue-100 px-2 py-0.5 text-xs text-blue-700">{emp.position}</span>
                        </div>
                        <p className="mt-1 text-xs text-gray-500">{emp.id}</p>
              ...
              <div className="space-y-2 border-t border-gray-200 p-4">
                <button
                  ...
                  className="w-full rounded-lg bg-gray-100 px-4 py-2 text-sm font-medium text-gray-700 transition-colors hover:bg-gray-200 disabled:cursor-not-allowed disabled:opacity-50"
                >
            ...
            <div className="flex-1 overflow-y-auto">
              {roleTemplates.map(role => (
                <div
                  ...
                  className={`cursor-pointer border-b border-gray-100 p-4 transition-colors ${
                    selectedRole === role.id ? 'border-l-4 border-l-teal-500 bg-teal-50' : 'hover:bg-gray-50'
                  }`}
                >
              ...
              <div className="border-b border-gray-200 bg-white p-4">
                <div className="mb-4 flex items-center justify-between">
                  ...
                  <button
                    ...
                    className="whitespace-nowrap rounded-lg bg-teal-500 px-6 py-2 font-medium text-white transition-colors hover:bg-teal-600"
                  >
                ...
                <div className="flex flex-wrap gap-2">
                  <button
                    ...
                    className="whitespace-nowrap rounded-lg bg-blue-500 px-4 py-2 text-sm font-medium text-white transition-colors hover:bg-blue-600"
                  >
                  ...
                  <button
                    ...
                    className="whitespace-nowrap rounded-lg bg-red-500 px-4 py-2 text-sm font-medium text-white transition-colors hover:bg-red-600"
                  >
                  ...
                  <button
                    onClick={expandAll}
                    className="whitespace-nowrap rounded-lg bg-gray-500 px-4 py-2 text-sm font-medium text-white transition-colors hover:bg-gray-600"
                  >
              ...
              <div className="flex-1 overflow-y-auto p-6">
                <div className="space-y-4">
                  {allPermissions.map(category => (
                    <div key={category.categoryId} className="rounded-lg border border-gray-200 bg-white shadow-sm">
                      <div className="border-b border-gray-200 p-4">
                        <div className="flex items-center gap-3">
                          ...
                          >
                            <i
                              className={`ri-arrow-${expandedCategories.has(category.categoryId) ? 'down' : 'right'}-s-line text-xl`}
                            ></i>
                          </button>
                          <input
                            type="checkbox"
                            ...
                            className="h-5 w-5 cursor-pointer rounded text-teal-500 focus:ring-2 focus:ring-teal-500"
                          />
                          <span className="text-lg font-bold text-gray-800">📁 {category.categoryName}</span>
                        </div>
                      </div>
                          ...
                          {category.menus.map(menu => (
                            <div key={menu.menuId} className="ml-8">
                              <div className="mb-2 flex items-center gap-3">
                                ...
                                >
                                  <i
                                    className={`ri-arrow-${expandedMenus.has(menu.menuId) ? 'down' : 'right'}-s-line text-lg`}
                                  ></i>
                                </button>
                                <input
                                  type="checkbox"
                                  ...
                                  className="h-4 w-4 cursor-pointer rounded text-teal-500 focus:ring-2 focus:ring-teal-500"
                                />
                                <span className="font-semibold text-gray-700">📄 {menu.menuName}</span>
                              </div>

                              {expandedMenus.has(menu.menuId) && (
                                <div className="ml-8 space-y-3">
                                  {menu.screens.map(screen => (
                                    <div key={screen.screenId} className="rounded-lg bg-gray-50 p-3">
                                      <div className="mb-2 flex items-center gap-3">
                                        <input
                                          type="checkbox"
                                          checked={
