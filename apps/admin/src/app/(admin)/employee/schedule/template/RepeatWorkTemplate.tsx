/**
 * Description : RepeatWorkTemplate.tsx - ?? RepeatWorkTemplate UI ????
 * Author : Shiwoo Min
 * Date : 2026-02-18
 */
...
// 아가페 표준 근무 코드 및 관제 색상 정의
const WORK_CODES = {
  S: { name: '주간', time: '07:00~19:00', color: 'bg-blue-50 text-blue-700 border-blue-200' },
...
/**
 * [Component] N일 주기 순환 근무 패턴 마스터 관리
 * 아가페 표준 UI 프로토콜(고딕/직각/한국어) 적용
 */
export default function RepeatWorkTemplate() {
  ...
  const loadTemplates = () => {
    const saved = localStorage.getItem('repeat_work_templates');
    if (saved) {
      try {
  ...
  const saveTemplates = (newTemplates: RepeatTemplate[]) => {
    localStorage.setItem('repeat_work_templates', JSON.stringify(newTemplates));
    setTemplates(newTemplates);
  };

  const handleDelete = (id: string) => {
    if (confirm('이 반복 근무 프로토콜을 영구 삭제하시겠습니까?')) {
    ...
    <div className="space-y-6 font-sans text-gray-900 antialiased">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2">
          <i className="ri-loop-right-line text-xl text-[#5C8D5A]"></i>
          <h3 className="text-[15px] font-black uppercase tracking-tight text-gray-800">순환 반복 시퀀스 목록</h3>
        </div>
        <button
          onClick={() => {
      ...
      <div className="grid grid-cols-1 gap-6 lg:grid-cols-2">
        {templates.map(template => (
          <div
            key={template.id}
            className="group overflow-hidden rounded-none border border-gray-300 bg-white shadow-sm transition-all hover:border-[#5C8D5A]"
          >
            {/* 카드 헤더 */}
            <div className="flex items-center justify-between border-b border-gray-200 bg-gray-50 p-4">
              <div>
                ...
                <span className="text-[10px] font-black uppercase italic tracking-widest text-[#5C8D5A]">
                  {template.repeatDays}-Day Rotation Protocol
                </span>
              </div>
              <div className="flex gap-1 opacity-40 transition-opacity group-hover:opacity-100">
                <button
                  onClick={() => {
                    setEditingTemplate(template);
                    setShowModal(true);
                  }}
                  className="cursor-pointer rounded-none p-2 text-gray-400 transition-all hover:bg-emerald-50 hover:text-[#5C8D5A]"
                >
                  <i className="ri-edit-line text-lg"></i>
                </button>
                <button
                  onClick={() => handleDelete(template.id)}
                  className="cursor-pointer rounded-none p-2 text-gray-400 transition-all hover:bg-red-50 hover:text-red-600"
                >
            ...
            <div className="space-y-4 p-5">
              ...
              <div className="grid grid-cols-2 gap-4 border border-gray-100 bg-gray-50 p-3 shadow-inner">
                <DataRow label="시퀀스 시작" value={template.startDate} isMono />
                <DataRow label="시퀀스 종료" value={template.endDate} isMono />
              </div>

              {/* 순환 시퀀스 상세 (고밀도 리스트) */}
              <div className="space-y-2">
                <label className="text-[10px] font-black uppercase italic tracking-widest text-gray-400">
                  Rotation Cycle Sequence
                </label>
                <div className="grid grid-cols-2 gap-2">
                  {template.pattern.map((p, index) => {
                    ...
                    return (
                      ...
                      >
                        <span className="w-10 text-[11px] font-black italic text-gray-400">{p.day}일차</span>
                        <span
                          className={clsx(
                        ...
                        <span className="font-mono text-[11px] font-bold italic text-gray-600">
                          {p.workType !== '휴' ? p.startTime : '-'}
                        </span>
                      </div>
                    );
                  })}
                ...
                <div className="flex flex-wrap gap-1.5">
                  {template.assignedStaff.length > 0 ? (
                    template.assignedStaff.map((staff, idx) => (
                      <span
                        key={idx}
                        className="rounded-none border border-gray-200 bg-gray-100 px-2 py-0.5 text-[10px] font-bold text-gray-600"
                      >
                        {staff}
                      </span>
                    ))
                  ) : (
                    <span className="text-[11px] font-bold italic text-gray-300">배정된 가용 인력 없음</span>
                  )}
                </div>
              </div>

              <button
                onClick={() => alert(`"${template.name}" 순환 로직을 활성화합니다.`)}
      ...
      {templates.length === 0 && (
        <div className="animate-in fade-in rounded-none border-2 border-dashed border-gray-200 bg-white p-20 text-center duration-500">
          <i className="ri-loop-right-line mb-4 block text-6xl text-gray-100"></i>
          <p className="text-[13px] font-bold italic text-gray-400">등록된 순환 반복 프로토콜이 존재하지 않습니다.</p>
        </div>
      )}
...
/** [Modal] 순환 템플릿 정밀 편집기 */
function RepeatTemplateModal({ template, onClose, onSave }: any) {
  const [formData, setFormData] = useState({
    name: template?.name || '',
    repeatDays: template?.repeatDays || 6,
  ...
  const updateCycleDays = (days: number) => {
    const nextDays = Math.max(1, Math.min(31, days));
    setFormData({ ...formData, repeatDays: nextDays });

    const newPattern = Array.from({ length: nextDays }, (_, i) => {
      const existing = pattern.find(p => p.day === i + 1);
      return existing || { day: i + 1, workType: 'S', startTime: '09:00', endTime: '18:00', breakTime: 60 };
    });
  ...
  return (
    <div className="fixed inset-0 z-[300] flex items-center justify-center bg-black/60 p-4 font-sans text-gray-900 antialiased backdrop-blur-sm">
      <div className="animate-in fade-in zoom-in flex max-h-[90vh] w-full max-w-4xl flex-col rounded-none border-2 border-[#5C8D5A] bg-white shadow-2xl duration-200">
        {/* 모달 헤더 */}
        ...
        <div className="custom-scrollbar flex-1 space-y-8 overflow-y-auto p-8">
          ...
          <div className="grid grid-cols-1 gap-6 md:grid-cols-2">
            <div className="space-y-2 md:col-span-2">
              ...
              <input
                ...
                placeholder="예: [요양팀] 주야비비비 5일 순환 모델"
              />
            </div>
            <div className="space-y-2">
              <label className="text-[11px] font-black uppercase italic tracking-widest text-gray-500">
              ...
              <input
                ...
                onChange={e => updateCycleDays(parseInt(e.target.value) || 0)}
                className="w-full rounded-none border-2 border-gray-100 bg-gray-50 p-3 font-mono text-[13px] font-black shadow-inner outline-none focus:border-[#5C8D5A] focus:bg-white"
              />
            </div>
            <div className="grid grid-cols-2 gap-2">
              <div className="space-y-2">
                ...
                <input
                  ...
                  className="w-full rounded-none border-2 border-gray-100 bg-gray-50 p-3 font-mono text-[12px] font-bold outline-none focus:border-[#5C8D5A]"
                />
              </div>
              <div className="space-y-2">
                <label className="text-[11px] font-black uppercase italic tracking-widest text-gray-500">
          ...
          <div className="space-y-3">
            <label className="text-[11px] font-black uppercase italic tracking-widest text-gray-500">
              순환 일자별 근무 정의 (Sequence Definition)
            </label>
            <div className="space-y-3 rounded-none border border-gray-100 bg-gray-50 p-6 shadow-inner">
              {pattern.map((p, idx) => (
                <div
                  key={idx}
                ...
                >
                  <span className="w-16 text-[12px] font-black italic text-gray-400">{p.day}일차</span>
                  <select
                    value={p.workType}
                    onChange={e => {
                      const next = [...pattern];
                    ...
                    className="min-w-[130px] cursor-pointer rounded-none border border-gray-300 bg-white p-2 text-[12px] font-black text-gray-800 outline-none focus:border-[#5C8D5A]"
                  >
                    ...
                    <div className="animate-in fade-in slide-in-from-left-2 flex items-center gap-3 duration-300">
                      <div className="flex items-center gap-2">
                        <input
                          ...
                          className="rounded-none border border-gray-200 p-2 font-mono text-[12px] font-bold outline-none focus:border-[#5C8D5A]"
                        />
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        <input
                          ...
                          onChange={e => {
                            const next = [...pattern];
                            next[idx].breakTime = parseInt(e.target.value) || 0;
                            setPattern(next);
                          }}
                          className="w-16 rounded-none border border-gray-200 p-2 text-right font-mono text-[12px] font-bold outline-none focus:border-[#5C8D5A]"
                        />
