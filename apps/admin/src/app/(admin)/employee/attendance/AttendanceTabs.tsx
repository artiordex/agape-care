/**
 * Description : AttendanceTabs.tsx - ?? AttendanceTabs UI ????
 * Author : Shiwoo Min
 * Date : 2026-02-18
 */
...
interface Props {
  readonly activeTab: 'daily' | 'monthly' | 'calendar';
  readonly onTabChange: (tab: 'daily' | 'monthly' | 'calendar') => void;
}
...
export default function AttendanceTabs({ activeTab, onTabChange }: Props) {
  const tabs = [
    { id: 'daily', label: '1. 일일 출퇴근 상세 기록', icon: 'ri-calendar-check-line' },
    { id: 'monthly', label: '월별 근태 정산 현황', icon: 'ri-medal-line' },
    { id: 'calendar', label: '출퇴근 근무표', icon: 'ri-calendar-event-line' },
  ] as const;
