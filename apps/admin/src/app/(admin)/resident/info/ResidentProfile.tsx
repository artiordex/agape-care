...
export default function ResidentProfile({ resident, getStatusColor, getGradeColor, onDownloadContract }: any) {
  const router = useRouter();

  if (!resident) return null;

  const handleEdit = () => {
    router.push(`/resident/info/edit?id=${resident.id}`);
  };

  const handleViewDetail = () => {
    router.push(`/resident/info/detail?id=${resident.id}`);
  };
      ...
      <div className="px-6 py-4">
        <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
          ...
          <div className="flex flex-wrap gap-2">
            ...
            >
              <i className="ri-edit-line text-base"></i>수정
            </button>
            ...
            <button
              ...
              className="flex items-center gap-1 rounded border border-gray-300 bg-white px-3 py-1.5 text-sm font-medium text-gray-700 transition-colors hover:bg-gray-50"
            >
              <i className="ri-file-text-line text-base"></i>계약서
            </button>

            <button
              onClick={handleViewDetail}
              className="flex items-center gap-1 rounded border border-blue-600 bg-blue-600 px-3 py-1.5 text-sm font-medium text-white transition-colors hover:bg-blue-700"
            >
              <i className="ri-search-eye-line text-base"></i>상세
            </button>
          </div>
        </div>
      </div>
      ...
      <div className="border-t border-gray-100 bg-gray-50 px-6 py-3">
        <div className="flex flex-wrap items-center gap-6 text-sm">
          <div className="flex items-center gap-2">
            ...
            <span className="font-semibold text-gray-900">{resident.bloodPressure}</span>
          </div>

          <div className="flex items-center gap-2">
            <span className="text-xs text-gray-500">혈당</span>
            <span className="font-semibold text-gray-900">{resident.bloodSugar}</span>
          </div>

          <div className="flex items-center gap-2">
            <span className="text-xs text-gray-500">신장</span>
            <span className="font-semibold text-gray-900">{resident.height}cm</span>
          </div>

          <div className="flex items-center gap-2">
            <span className="text-xs text-gray-500">체중</span>
            <span className="font-semibold text-gray-900">{resident.weight}kg</span>
          </div>

          <div className="flex items-center gap-2">
            <span className="text-xs text-gray-500">등급만료</span>
            <span className="font-semibold text-gray-900">{resident.gradeValidUntil}</span>
          </div>
        </div>
      </div>
    </div>
