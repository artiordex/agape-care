/**
 * Description : ResidentFormModal.tsx - 📌 수급자 등록/수정 통합 모달
 * Author : Shiwoo Min
 * Date : 2026-02-18
 */

'use client';

import clsx from 'clsx';
import React, { useEffect, useState } from 'react';

export type ResidentFormMode = 'create' | 'edit';

interface Props {
  readonly isOpen: boolean;
  readonly mode: ResidentFormMode;
  readonly initialData?: any;
  readonly onClose: () => void;
  readonly onSave: (data: any) => void;
  readonly onDelete?: (id: string) => void;
}

export default function ResidentFormModal({ isOpen, mode, initialData, onClose, onSave, onDelete }: Props) {
  const [formData, setFormData] = useState({
    name: '',
    gender: '남',
    birthDate: '',
    birthType: '음력',
    grade: '',
    isGradeOut: false,
    gradeFrom: '',
    gradeTo: '',
    gradePeriod: '2년',
    certNo: '',
    admissionDate: '',
    admissionTime: '10:00',
    copaymentRate: '',
    phone: '',
    tel: '',
    zipCode: '',
    address: '',
    addressDetail: '',
    disease: '',
    room: '',
    supportMeal: false,
    supportSnack: false,
    memo: '',
    // Guardian
    guardianName: '',
    guardianRelation: '',
    guardianPhone: '',
    guardianEmail: '',
    guardianBirth: '',
    guardianTel: '',
    billingMethod: ['문자.알림톡'],
    billingZipCode: '',
    billingAddress: '',
    family: [
      { name: '', relation: '', phone: '', isPay: false, isBill: false },
      { name: '', relation: '', phone: '', isPay: false, isBill: false },
    ],
  });

  useEffect(() => {
    if (isOpen && mode === 'edit' && initialData) {
      // Map initialData (from residents summary or detail) to form structure
      setFormData(prev => ({
        ...prev,
        ...initialData,
        // Ensure some fields are mapped correctly if names differ
        name: initialData.name || '',
        gender: initialData.gender || '남',
        birthDate: initialData.birthDate || '',
        grade: initialData.grade || '',
        certNo: initialData.registrationNumber || '',
        admissionDate: initialData.admissionDate || '',
        room: initialData.room || '',
      }));
    } else if (isOpen && mode === 'create') {
      // Reset to default
      setFormData({
        name: '',
        gender: '남',
        birthDate: '2026.02.18',
        birthType: '음력',
        grade: '',
        isGradeOut: false,
        gradeFrom: '',
        gradeTo: '',
        gradePeriod: '2년',
        certNo: '',
        admissionDate: '2026.02.18',
        admissionTime: '14:30',
        copaymentRate: '',
        phone: '',
        tel: '',
        zipCode: '',
        address: '',
        addressDetail: '',
        disease: '',
        room: '',
        supportMeal: false,
        supportSnack: false,
        memo: '',
        guardianName: '',
        guardianRelation: '',
        guardianPhone: '',
        guardianEmail: '',
        guardianBirth: '',
        guardianTel: '',
        billingMethod: ['문자.알림톡'],
        billingZipCode: '',
        billingAddress: '',
        family: [
          { name: '', relation: '', phone: '', isPay: false, isBill: false },
          { name: '', relation: '', phone: '', isPay: false, isBill: false },
        ],
      });
    }
  }, [isOpen, mode, initialData]);

  if (!isOpen) return null;

  const handleInputChange = (e: React.ChangeEvent<HTMLInputElement | HTMLSelectElement | HTMLTextAreaElement>) => {
    const { name, value, type } = e.target;
    if (type === 'checkbox') {
      const checked = (e.target as HTMLInputElement).checked;
      setFormData(prev => ({ ...prev, [name]: checked }));
    } else {
      setFormData(prev => ({ ...prev, [name]: value }));
    }
  };

  const labelClass =
    'bg-[#E8F1F8] border border-[#B8D1E0] px-2 py-1.5 text-center text-[12px] font-bold text-gray-700 w-[100px] shrink-0 flex items-center justify-center';
  const inputClass = 'border border-[#B8D1E0] px-2 py-1 text-[12px] outline-none focus:border-blue-400 w-full h-[28px]';
  const selectClass = 'border border-[#B8D1E0] px-1 py-1 text-[12px] outline-none focus:border-blue-400 h-[28px]';
  const checkboxLabelClass = 'flex items-center gap-1 text-[12px] cursor-pointer';

  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/50 p-4 antialiased">
      <div className="w-full max-w-[1000px] overflow-hidden rounded-sm border border-blue-900 bg-white shadow-2xl">
        {/* Header */}
        <div className="flex items-center justify-between border-b border-[#B8D1E0] bg-[#E3F2FD] px-4 py-2">
          <h2 className="text-[16px] font-black tracking-tight text-gray-800">
            {mode === 'create' ? '수급자 신규등록' : '수급자 기본정보 수정'}
          </h2>
          <button onClick={onClose} className="text-gray-500 transition-colors hover:text-red-500">
            <i className="ri-close-line text-2xl"></i>
          </button>
        </div>

        {/* Content */}
        <div className="max-h-[85vh] overflow-y-auto bg-white p-4">
          {/* Section 1: 수급자 기본 정보 */}
          <div className="mb-6">
            <div className="mb-2 flex items-center gap-1 text-[14px] font-bold text-[#2E6A9E]">
              <i className="ri-checkbox-indeterminate-line"></i>
              <span>수급자 기본 정보</span>
            </div>

            <div className="flex border-collapse border border-[#B8D1E0]">
              {/* Photo Section */}
              <div className="flex w-[160px] shrink-0 flex-col items-center border-r border-[#B8D1E0] p-3">
                <div className="mb-2 flex h-[150px] w-[120px] items-center justify-center border border-dashed border-gray-300 bg-gray-50">
                  <i className="ri-user-fill text-6xl text-gray-200"></i>
                </div>
                <button className="mb-1 rounded-sm bg-gray-500 px-3 py-1 text-[11px] text-white">사진선택</button>
                <p className="text-[9px] text-gray-400">(직원 앱에서도 등록가능)</p>
              </div>

              {/* Grid Section */}
              <div className="grid flex-1 grid-cols-12">
                {/* Row 1 */}
                <div className={clsx(labelClass, 'col-span-2')}>
                  수급자명<span className="ml-0.5 text-red-500">*</span>
                </div>
                <div className="col-span-4 flex items-center gap-2 border border-[#B8D1E0] p-1">
                  <input
                    type="text"
                    className={clsx(inputClass, 'w-[120px]')}
                    value={formData.name}
                    name="name"
                    onChange={handleInputChange}
                  />
                  <div className="flex items-center gap-3">
                    <label className={checkboxLabelClass}>
                      <input
                        type="radio"
                        name="gender"
                        value="남"
                        checked={formData.gender === '남'}
                        onChange={handleInputChange}
                      />{' '}
                      남
                    </label>
                    <label className={checkboxLabelClass}>
                      <input
                        type="radio"
                        name="gender"
                        value="여"
                        checked={formData.gender === '여'}
                        onChange={handleInputChange}
                      />{' '}
                      여
                    </label>
                  </div>
                </div>
                <div className={clsx(labelClass, 'col-span-2')}>
                  생년월일<span className="ml-0.5 text-red-500">*</span>
                </div>
                <div className="col-span-4 flex items-center gap-2 border border-[#B8D1E0] p-1">
                  <input
                    type="text"
                    placeholder="YYYY.MM.DD"
                    className={clsx(inputClass, 'w-[120px]')}
                    value={formData.birthDate}
                    name="birthDate"
                    onChange={handleInputChange}
                  />
                  <i className="ri-calendar-event-line text-gray-400"></i>
                </div>

                {/* Row: Birthday Type attached to Row 1 logic in image */}
                <div className={clsx(labelClass, 'col-span-2')}>생신일</div>
                <div className="col-span-10 flex items-center gap-3 border border-[#B8D1E0] p-1">
                  <input type="text" placeholder="YYYY.MM.DD" className={clsx(inputClass, 'w-[120px]')} />
                  <i className="ri-calendar-event-line text-gray-400"></i>
                  <div className="ml-2 flex items-center gap-3">
                    <label className={checkboxLabelClass}>
                      <input
                        type="radio"
                        name="birthType"
                        value="양력"
                        checked={formData.birthType === '양력'}
                        onChange={handleInputChange}
                      />{' '}
                      양력
                    </label>
                    <label className={checkboxLabelClass}>
                      <input
                        type="radio"
                        name="birthType"
                        value="음력"
                        checked={formData.birthType === '음력'}
                        onChange={handleInputChange}
                      />{' '}
                      음력
                    </label>
                  </div>
                </div>

                {/* Row 2 */}
                <div className={clsx(labelClass, 'col-span-2')}>
                  인정등급<span className="ml-0.5 text-red-500">*</span>
                </div>
                <div className="col-span-4 flex items-center gap-2 border border-[#B8D1E0] p-1">
                  {mode === 'edit' ? (
                    <input type="text" className={inputClass} value={formData.grade} readOnly />
                  ) : (
                    <select
                      className={clsx(selectClass, 'w-[100px]')}
                      name="grade"
                      value={formData.grade}
                      onChange={handleInputChange}
                    >
                      <option>--선택--</option>
                      <option>1등급</option>
                      <option>2등급</option>
                      <option>3등급</option>
                    </select>
                  )}
                  <label className={checkboxLabelClass}>
                    <input
                      type="checkbox"
                      name="isGradeOut"
                      checked={formData.isGradeOut}
                      onChange={handleInputChange}
                    />{' '}
                    등급외
                  </label>
                </div>
                <div className={clsx(labelClass, 'col-span-2')}>
                  인정기간<span className="ml-0.5 text-red-500">*</span>
                </div>
                <div className="col-span-4 flex items-center gap-1 border border-[#B8D1E0] p-1">
                  <input
                    type="text"
                    placeholder="YYYY.MM.DD"
                    className={clsx(inputClass, 'w-[100px]')}
                    value={formData.gradeFrom}
                    name="gradeFrom"
                    onChange={handleInputChange}
                  />
                  <span>~</span>
                  <input
                    type="text"
                    placeholder="YYYY.MM.DD"
                    className={clsx(inputClass, 'w-[100px]')}
                    value={formData.gradeTo}
                    name="gradeTo"
                    onChange={handleInputChange}
                  />
                  <select
                    className={clsx(selectClass, 'w-16')}
                    name="gradePeriod"
                    value={formData.gradePeriod}
                    onChange={handleInputChange}
                  >
                    <option>2년</option>
                    <option>1년</option>
                  </select>
                </div>

                {/* Row 3 */}
                <div className={clsx(labelClass, 'col-span-2')}>
                  입소일시<span className="ml-0.5 text-red-500">*</span>
                </div>
                <div className="col-span-4 flex items-center gap-2 border border-[#B8D1E0] p-1">
                  <input
                    type="text"
                    className={clsx(inputClass, 'w-[100px]')}
                    value={formData.admissionDate}
                    name="admissionDate"
                    onChange={handleInputChange}
                  />
                  <i className="ri-calendar-event-line text-gray-400"></i>
                  <input
                    type="text"
                    className={clsx(inputClass, 'w-10')}
                    value={formData.admissionTime.split(':')[0]}
                  />{' '}
                  :{' '}
                  <input
                    type="text"
                    className={clsx(inputClass, 'w-10')}
                    value={formData.admissionTime.split(':')[1]}
                  />
                </div>
                <div className={clsx(labelClass, 'col-span-2')}>
                  본인부담률<span className="ml-0.5 text-red-500">*</span>
                </div>
                <div className="col-span-4 flex items-center border border-[#B8D1E0] p-1">
                  {mode === 'edit' ? (
                    <input
                      type="text"
                      className={inputClass}
                      value={formData.copaymentRate ? `${formData.copaymentRate}%` : '12% (감경40%)'}
                      readOnly
                    />
                  ) : (
                    <select
                      className={clsx(selectClass, 'w-[120px]')}
                      name="copaymentRate"
                      value={formData.copaymentRate}
                      onChange={handleInputChange}
                    >
                      <option>--선택--</option>
                      <option value="20">20%</option>
                      <option value="12">12%</option>
                    </select>
                  )}
                </div>

                {/* Row 4: 인정번호 / 연락처 */}
                <div className={labelClass + ' col-span-2'}>
                  인정번호<span className="ml-0.5 text-red-500">*</span>
                </div>
                <div className="col-span-4 flex items-center gap-1 border border-[#B8D1E0] p-1">
                  <input
                    type="text"
                    className={clsx(inputClass, 'w-full')}
                    value={formData.certNo}
                    name="certNo"
                    onChange={handleInputChange}
                  />
                </div>
                <div className={labelClass + ' col-span-2'}>연락처</div>
                <div className="col-span-4 flex items-center gap-1 border border-[#B8D1E0] p-1">
                  <input
                    type="text"
                    placeholder="휴대폰번호"
                    className={inputClass}
                    value={formData.phone}
                    name="phone"
                    onChange={handleInputChange}
                  />
                  <input
                    type="text"
                    placeholder="전화번호"
                    className={inputClass}
                    value={formData.tel}
                    name="tel"
                    onChange={handleInputChange}
                  />
                </div>

                {/* Row 5: 주소 */}
                <div className={clsx(labelClass, 'col-span-2 h-16')}>주소</div>
                <div className="col-span-6 flex flex-col gap-1 border border-[#B8D1E0] p-1">
                  <div className="flex gap-1">
                    <input type="text" className={clsx(inputClass, 'w-20')} value={formData.zipCode} readOnly />
                    <button className="rounded-sm bg-[#8FA1B0] px-2 py-1 text-[11px] text-white">우편번호</button>
                    <input type="text" className={clsx(inputClass, 'flex-1')} value={formData.address} readOnly />
                  </div>
                  <div className="flex gap-1">
                    <input
                      type="text"
                      placeholder="상세주소"
                      className={inputClass}
                      value={formData.addressDetail}
                      name="addressDetail"
                      onChange={handleInputChange}
                    />
                    <button className="shrink-0 rounded-sm bg-gray-400 px-2 py-1 text-[11px] text-white">지움</button>
                  </div>
                </div>
                <div className={clsx(labelClass, 'col-span-2 h-16')}>주요질환</div>
                <div className="col-span-2 border border-[#B8D1E0] p-1">
                  <textarea
                    className="h-full w-full resize-none border-none text-[12px] outline-none"
                    value={formData.disease}
                    name="disease"
                    onChange={handleInputChange}
                  ></textarea>
                </div>

                {/* Row 6: 생활실 / 생계급여지원 */}
                <div className={clsx(labelClass, 'col-span-2')}>생활실</div>
                <div className="col-span-4 flex items-center gap-1 border border-[#B8D1E0] p-1">
                  <input type="text" className={clsx(inputClass, 'flex-1')} value={formData.room} readOnly />
                  <button className="shrink-0 rounded-sm bg-[#8FA1B0] px-2 py-1 text-[11px] text-white">
                    생활실 선택
                  </button>
                </div>
                <div className={clsx(labelClass, 'col-span-2')}>생계급여 지원</div>
                <div className="col-span-4 flex items-center gap-4 border border-[#B8D1E0] p-1">
                  <label
                    className={clsx(checkboxLabelClass, 'rounded-[2px] border border-green-600 px-1 text-green-700')}
                  >
                    <input
                      type="checkbox"
                      name="supportMeal"
                      checked={formData.supportMeal}
                      onChange={handleInputChange}
                      className="hidden"
                    />{' '}
                    식사재료비
                  </label>
                  <label
                    className={clsx(checkboxLabelClass, 'rounded-[2px] border border-green-600 px-1 text-green-700')}
                  >
                    <input
                      type="checkbox"
                      name="supportSnack"
                      checked={formData.supportSnack}
                      onChange={handleInputChange}
                      className="hidden"
                    />{' '}
                    간식비
                  </label>
                </div>

                {/* Row 7: 비고 */}
                <div className={clsx(labelClass, 'col-span-12 h-6 justify-start bg-[#F8FAFC] px-4')}> </div>
                <div className={clsx(labelClass, 'col-span-2')}>비고</div>
                <div className="col-span-10 border border-[#B8D1E0] p-1">
                  <textarea
                    placeholder="수급자 비고"
                    className="h-10 w-full resize-none border-none text-[12px] outline-none"
                    value={formData.memo}
                    name="memo"
                    onChange={handleInputChange}
                  ></textarea>
                </div>
              </div>
            </div>
          </div>

          {/* Guardian section is present in image but less prominent in request for detail view,
               keeping it for consistency if needed, but focusing on the main grid layout mapping. */}
        </div>

        {/* Footer */}
        <div className="flex justify-center gap-3 border-t border-[#B8D1E0] bg-[#F8FAFC] p-3">
          <button
            onClick={() => onSave(formData)}
            className="w-28 rounded-md border border-blue-700 bg-gradient-to-b from-blue-400 to-blue-600 px-1 py-1.5 text-[14px] font-bold text-white shadow-md transition-all hover:from-blue-500 hover:to-blue-700 active:scale-95"
          >
            저장
          </button>

          {mode === 'edit' && (
            <button
              onClick={() => onDelete?.(initialData?.id)}
              className="w-28 rounded-md border border-red-700 bg-gradient-to-b from-red-400 to-red-600 px-1 py-1.5 text-[14px] font-bold text-white shadow-md transition-all hover:from-red-500 hover:to-red-700 active:scale-95"
            >
              삭제
            </button>
          )}

          <button
            onClick={onClose}
            className="w-28 rounded-md border border-gray-800 bg-gradient-to-b from-gray-500 to-gray-700 px-1 py-1.5 text-[14px] font-bold text-white shadow-md transition-all hover:from-gray-600 hover:to-gray-800 active:scale-95"
          >
            창닫기
          </button>
        </div>
      </div>
    </div>
  );
}
